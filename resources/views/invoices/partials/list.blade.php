                ...
                <tbody class="divide-y divide-slate-700/30">
                    @foreach($invoices as $invoice)
                        <tr class="hover:bg-slate-700/20 transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap"><span class="px-2 py-0.5 rounded text-xs bg-slate-600/50 text-slate-300">Project</span></td>
                    ...
                    @foreach(($supportInvoices ?? collect()) as $sp)
                        <tr class="hover:bg-slate-700/20 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap"><div class="text-sm font-medium text-white">{{ $sp->invoice_number }}</div></td>
                            <td class="px-6 py-4">
                                <div class="text-sm text-white">{{ $sp->project->project_name }}</div>
                                <div class="text-xs text-slate-400">{{ $sp->project->project_code ?? '' }}</div>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap"><div class="text-sm text-slate-300">{{ $sp->paid_at?->format('M d, Y') ?? '—' }}</div></td>
                            <td class="px-6 py-4 whitespace-nowrap"><div class="text-sm font-medium text-white">৳ {{ number_format($sp->amount, 2) }}</div></td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('support-packages.view-invoice', [$sp->project, $sp]) }}" target="_blank" class="px-3 py-1.5 rounded-lg bg-sky-500/20 text-sky-400 hover:bg-sky-500/30 text-xs font-medium inline-flex items-center gap-1">View</a>
...
@else
    <div class="bg-slate-800/60 border border-slate-700/50 rounded-2xl p-12 text-center">
        ...
        <p class="text-slate-400">Invoices will appear here when payments are recorded for your projects (including support packages).</p>
    </div>
@endif
