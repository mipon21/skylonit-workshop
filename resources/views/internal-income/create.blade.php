<x-app-layout>
    ...
    <div class="space-y-6">
        <div>
            ...
            <h1 class="text-2xl font-semibold text-white">Add external income</h1>
        </div>

        <p class="text-slate-400 text-sm">Add external income to Overhead or Profit fund. This increases the fund balance for internal expenses.</p>
            ...
            <div>
                <form action="{{ route('internal-income.store') }}" method="POST" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="title" id="title" value="{{ old('title') }}" required class="w-full rounded-xl bg-slate-800 border border-slate-600 text-white px-4 py-2.5 focus:ring-2 focus:ring-sky-500 focus:border-sky-500" placeholder="e.g. Client support package, Consulting fee">
                    ...
                    <div>
                        ...
                        <input type="date" name="income_date" id="income_date" value="{{ old('income_date', now()->format('Y-m-d')) }}" required class="w-full rounded-xl bg-slate-800 border border-slate-600 text-white px-4 py-2.5 focus:ring-2 focus:ring-sky-500 focus:border-sky-500">
                    ...
                    <div>
                        ...
                        <div class="space-y-2">
                            <label class="flex items-center gap-3 p-3 rounded-xl border border-slate-600 hover:bg-slate-800/50 cursor-pointer">
                                <input type="radio" name="fund_type" value="overhead" {{ old('fund_type', 'overhead') === 'overhead' ? 'checked' : '' }} class="rounded border-slate-600 bg-slate-800 text-sky-500 focus:ring-sky-500">
                                <span class="text-slate-300">Overhead</span>
                                <span class="text-slate-500 text-sm">(balance: ৳{{ number_format($overheadBalance, 0) }})</span>
                            </label>
                            <label class="flex items-center gap-3 p-3 rounded-xl border border-slate-600 hover:bg-slate-800/50 cursor-pointer">
                                <input type="radio" name="fund_type" value="profit" {{ old('fund_type') === 'profit' ? 'checked' : '' }} class="rounded border-slate-600 bg-slate-800 text-sky-500 focus:ring-sky-500">
                    ...
                    <div class="flex gap-3 pt-2">
                        ...
                        <button type="submit" class="px-4 py-2.5 rounded-xl bg-sky-500 hover:bg-sky-600 text-white font-medium">Record income</button>
                    </div>
                </form>
            </div>
        </div>
