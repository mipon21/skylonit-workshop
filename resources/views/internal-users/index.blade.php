<x-app-layout>
    ...
    <div class="space-y-6" x-data="{ open: {{ json_encode($errors->any()) }} }">
        <div class="flex flex-wrap items-center justify-between gap-4">
            <h1 class="text-2xl font-semibold theme-text-primary">{{ $roleLabel }}s</h1>
            @php
                $createRoute = $role === 'developer' ? 'developers.create' : 'sales.create';
            @endphp
            <a href="{{ route($createRoute) }}" class="px-4 py-2.5 rounded-xl theme-btn-primary font-medium text-sm transition">
                Add {{ $roleLabel }}
            </a>
        </div>
        ...
        <div class="space-y-6" x-data="internalUsersPage()">
            ...
            <input type="text" id="internal-users-search" x-model="searchText" placeholder="Search by name, email…" class="w-full max-w-md rounded-xl theme-bg-tertiary border theme-border theme-text-primary px-4 py-2.5 text-sm theme-input-placeholder theme-input-focus">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full max-md:min-w-[400px]">
                        ...
                        <tbody class="divide-y divide-slate-700/50">
                            @forelse($users as $u)
                                <tr class="hover:theme-bg-tertiary/40 transition" x-show="filteredIds.includes({{ $u->id }})" x-transition>
                                    <td class="px-5 py-4">
                                        @php $showRoute = $role === 'developer' ? 'developers.show' : 'sales.show'; @endphp
                                        <a href="{{ route($showRoute, $u) }}" class="font-medium text-orange-400 hover:text-orange-300">{{ $u->name }}</a>
                                    </td>
                                    <td class="px-5 py-4 theme-text-secondary">{{ $u->email }}</td>
                                    <td class="px-5 py-4 text-right">
                                        ...
                                        <a href="{{ route($editRoute, $u) }}" class="theme-text-secondary theme-hover-primary text-sm mr-3">Edit</a>
                            ...
                            @empty
                                <tr>
                                    <td colspan="3" class="px-5 py-12 text-center theme-text-muted">No {{ $roleLabel }}s yet. Add one to get started.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
            ...
            <p x-show="usersData.length && searchText && filteredIds.length === 0" x-transition class="py-6 text-center theme-text-muted">No {{ $roleLabel }}s match your search.</p>
        </div>
    </div>
</x-app-layout>
