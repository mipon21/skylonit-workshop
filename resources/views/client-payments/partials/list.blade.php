@php $supportPackages = $supportPackages ?? collect(); @endphp
@if($payments->count() > 0 || $supportPackages->count() > 0)
    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 max-md:grid-cols-1 max-md:gap-3">
        ...
        @endforeach
        @foreach($supportPackages as $sp)
            <div class="rounded-2xl bg-slate-800/60 border border-slate-700/50 p-5 flex flex-col max-md:p-4 border-l-4 border-l-violet-500/60">
                <div class="flex items-start justify-between gap-2">
                    <div>
                        <p class="font-semibold text-white">{{ $sp->project->project_name }}</p>
                        <p class="text-slate-400 text-sm">{{ $sp->package_label ?? 'Support' }}</p>
                    </div>
                    <span @class([
                        'shrink-0 px-2.5 py-1 rounded-lg text-xs font-medium',
                        'bg-amber-500/20 text-amber-400 border border-amber-500/40' => $sp->payment_status === 'due',
                        'bg-emerald-500/20 text-emerald-400 border border-emerald-500/40' => $sp->payment_status === 'paid',
                    ])>{{ strtoupper($sp->payment_status) }}</span>
                </div>
                <p class="text-xl font-bold text-white mt-2">৳ {{ number_format($sp->amount, 0) }}</p>
                <p class="text-slate-500 text-xs mt-0.5">Support package</p>
                <p class="text-slate-500 text-xs mt-2">
                    {{ $sp->payment_status === 'paid' && $sp->paid_at ? 'Paid ' . $sp->paid_at->format('M d, Y') : 'Created ' . $sp->created_at->format('M d, Y') }}
                </p>
                <div class="mt-4 pt-4 border-t border-slate-700/50 flex flex-col gap-2">
                    @if($sp->payment_status === 'due' && $sp->payment_link)
                        <a href="{{ $sp->payment_link }}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-sky-500 hover:bg-sky-600 text-white font-medium text-sm transition max-md:w-full">Pay Now</a>
                    @endif
                    @if($sp->payment_status === 'paid' && $sp->invoice_path)
                        <a href="{{ route('support-packages.download-invoice', [$sp->project, $sp]) }}" class="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-xl border border-slate-600 text-slate-300 hover:bg-slate-700 text-sm font-medium transition max-md:w-full">Download Invoice</a>
                    @endif
                </div>
            </div>
        @endforeach
...
@else
    <div class="bg-slate-800/60 border border-slate-700/50 rounded-2xl p-12 text-center">
        ...
        <p class="text-slate-400">Payments for your projects (including support packages) will appear here. Use "Pay Now" to pay online when a payment is due.</p>
    </div>
@endif
