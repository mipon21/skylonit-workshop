    ...
    <div class="space-y-6">
        <div class="flex flex-wrap items-center justify-between gap-4">
            ...
            <div class="flex flex-wrap gap-2">
                ...
                <a href="{{ route('investments.create') }}" class="px-4 py-2.5 rounded-xl bg-sky-500 hover:bg-sky-600 text-white font-medium text-sm transition">Add Investor</a>
            </div>
        </div>

        <p class="text-slate-400 text-sm bg-slate-800/40 border border-slate-700/50 rounded-xl px-4 py-3">
            <span class="text-sky-400 font-medium">Note:</span> To consume profit, investors need to invest. Profit distributions are only available to investors who have an active investment.
        </p>
                    ...
                    <tbody class="divide-y divide-slate-700/50">
                        @forelse($investments as $inv)
                            <tr class="hover:bg-slate-800/40 transition">
                                ...
                                <td class="px-5 py-4 text-right text-slate-300">৳{{ number_format($inv->remaining_cap, 0) }}</td>
                                <td class="px-5 py-4">
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-5 py-12 text-center text-slate-500">No investors yet. Add one to track growth funding (profit sharing is capped and non-equity).</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
