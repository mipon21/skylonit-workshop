...
<div x-show="linkEditModal === {{ $link->id }}" x-cloak class="fixed inset-0 z-50 overflow-y-auto" aria-modal="true">
    <div class="flex min-h-full items-center justify-center p-4 max-md:p-0 max-md:items-stretch">
        ...
        <div x-show="linkEditModal === {{ $link->id }}" x-transition class="relative w-full max-w-md theme-bg-tertiary border theme-border rounded-2xl shadow-xl p-6 max-md:max-w-none max-md:max-h-full max-md:rounded-none max-md:border-0">
            <h2 class="text-lg font-semibold theme-text-primary mb-4">Edit Link / APK</h2>
            <form action="{{ route('projects.links.update', [$project, $link]) }}" method="POST" enctype="multipart/form-data" x-data="{ linkType: '{{ old('link_type', $link->link_type ?? 'url') }}' }">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="label" value="{{ old('label', $link->label) }}" required class="w-full rounded-xl theme-input-bg border theme-border theme-text-primary px-4 py-2.5 theme-input-focus">
                    ...
                    <template x-if="linkType === 'url'">
                        <div class="space-y-4">
                            <div>
                                ...
                                <input type="url" name="url" value="{{ old('url', $link->isApk() ? '' : $link->url) }}" class="w-full rounded-xl theme-input-bg border theme-border theme-text-primary px-4 py-2.5 theme-input-focus" placeholder="https://...">
                            ...
                            <div>
                                ...
                                <input type="text" name="login_username" value="{{ old('login_username', $link->login_username) }}" class="w-full rounded-xl theme-input-bg border theme-border theme-text-primary px-4 py-2.5">
                            </div>
                            <div>
                                <label class="block text-sm font-medium theme-text-secondary mb-1">Login password (optional)</label>
                                <input type="text" name="login_password" value="{{ old('login_password', $link->login_password) }}" class="w-full rounded-xl theme-input-bg border theme-border theme-text-primary px-4 py-2.5">
                            </div>
                        </div>
                    </template>
                    <template x-if="linkType === 'apk'">
                        <div>
                            @if($link->file_path)
                                <p class="theme-text-muted text-xs mb-1">Current file: {{ $link->file_name ?? basename($link->file_path) }}</p>
                            @endif
                            <label class="block text-sm font-medium theme-text-secondary mb-1">APK file {{ $link->file_path ? '(optional, replace)' : '*' }}</label>
                            <input type="file" name="apk_file" accept=".apk,.aab" class="w-full rounded-xl theme-input-bg border theme-border theme-text-primary px-4 py-2.5 file:mr-3 file:py-1.5 file:rounded-lg file:border-0 file:theme-bg-tertiary file:theme-text-secondary text-sm">
                    ...
                    <div class="pt-2 border-t theme-border">
                        ...
                        @if($isDeveloper ?? false)
                            ...
                            <p class="theme-text-muted text-xs mt-1">Developers can only set visibility to Admin & Client.</p>
                        @else
                ...
                <div class="mt-6 flex justify-end gap-3">
                    <button type="button" @click="linkEditModal = null" class="px-4 py-2.5 rounded-xl border theme-border theme-text-secondary theme-sidebar-link-hover">Cancel</button>
