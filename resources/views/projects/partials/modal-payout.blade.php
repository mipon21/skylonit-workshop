...
<div x-show="payoutModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto" aria-modal="true">
    <div class="flex min-h-full items-center justify-center p-4 max-md:p-0 max-md:items-stretch">
        ...
        <div x-show="payoutModal" x-transition class="relative w-full max-w-md theme-bg-tertiary border theme-border rounded-2xl shadow-xl p-6 max-md:max-w-none max-md:max-h-full max-md:rounded-none max-md:border-0">
            <h2 class="text-lg font-semibold theme-text-primary mb-4" x-text="payoutType ? 'Payout: ' + payoutType.charAt(0).toUpperCase() + payoutType.slice(1) : 'Payout'"></h2>
            ...
            @foreach($payoutTypes as $type => $label)
                ...
                <form action="{{ route('projects.payouts.update', $project) }}" method="POST" x-show="payoutType === '{{ $type }}'" x-transition class="space-y-4">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            <label class="block text-sm font-medium theme-text-secondary mb-1">Amount paid (৳)</label>
                            <input type="number" name="amount_paid" step="0.01" min="0" value="{{ $p?->amount_paid ?? '' }}" placeholder="Optional" class="w-full rounded-xl theme-input-bg border theme-border theme-text-primary px-4 py-2.5 theme-input-focus">
                        </div>
                        <div>
                            <label class="block text-sm font-medium theme-text-secondary mb-1">Paid at (date)</label>
                            <input type="date" name="paid_at" value="{{ $p?->paid_at?->format('Y-m-d') ?? '' }}" class="w-full rounded-xl theme-input-bg border theme-border theme-text-primary px-4 py-2.5 theme-input-focus">
                        </div>
                        <div>
                            <label class="block text-sm font-medium theme-text-secondary mb-1">Note</label>
                            <textarea name="note" rows="2" class="w-full rounded-xl theme-input-bg border theme-border theme-text-primary px-4 py-2.5 theme-input-focus" placeholder="Optional">{{ $p?->note ?? '' }}</textarea>
                        </div>
                    </div>
                    <div class="mt-6 flex justify-end gap-3">
                        <button type="button" @click="payoutModal = false" class="px-4 py-2.5 rounded-xl border theme-border theme-text-secondary theme-sidebar-link-hover">Cancel</button>
