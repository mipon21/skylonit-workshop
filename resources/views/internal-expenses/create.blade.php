<x-app-layout>
    ...
    <div class="space-y-6">
        <div>
            ...
            <h1 class="text-2xl font-semibold text-white">New internal expense</h1>
        </div>

        <p class="text-slate-400 text-sm">Company-wide costs. Default: pay from Overhead. If Overhead is insufficient, choose Investor Capital explicitly. All usage is logged and auditable.</p>
                    ...
                    <ul class="space-y-2 text-sm">
                        ...
                        @foreach($investments as $inv)
                            @if(($investmentBalances[$inv->id] ?? 0) > 0)
                                <li class="flex justify-between"><span class="text-slate-400">{{ $inv->investor_name }}</span><span class="text-emerald-400 font-medium">৳{{ number_format($investmentBalances[$inv->id] ?? 0, 0) }}</span></li>
                            @endif
                        @endforeach
                    </ul>
                </div>
            ...
            <div>
                <form action="{{ route('internal-expenses.store') }}" method="POST" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="title" id="title" value="{{ old('title') }}" required class="w-full rounded-xl bg-slate-800 border border-slate-600 text-white px-4 py-2.5 focus:ring-2 focus:ring-sky-500 focus:border-sky-500">
                        @error('title')<p class="mt-1 text-sm text-red-400">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        <label for="amount" class="block text-sm font-medium text-slate-300 mb-1">Amount (৳)</label>
                        <input type="number" name="amount" id="amount" value="{{ old('amount') }}" step="0.01" min="0.01" required class="w-full rounded-xl bg-slate-800 border border-slate-600 text-white px-4 py-2.5 focus:ring-2 focus:ring-sky-500 focus:border-sky-500">
                    ...
                    <div>
                        ...
                        <input type="date" name="expense_date" id="expense_date" value="{{ old('expense_date', now()->format('Y-m-d')) }}" required class="w-full rounded-xl bg-slate-800 border border-slate-600 text-white px-4 py-2.5 focus:ring-2 focus:ring-sky-500 focus:border-sky-500">
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="3" class="w-full rounded-xl bg-slate-800 border border-slate-600 text-white px-4 py-2.5 focus:ring-2 focus:ring-sky-500 focus:border-sky-500">{{ old('description') }}</textarea>
                    ...
                    <div>
                        ...
                        <div class="space-y-2">
                            <label class="flex items-center gap-3 p-3 rounded-xl border border-slate-600 hover:bg-slate-800/50 cursor-pointer">
                                <input type="radio" name="funded_from" value="overhead" {{ old('funded_from', 'overhead') === 'overhead' ? 'checked' : '' }} class="rounded border-slate-600 bg-slate-800 text-sky-500 focus:ring-sky-500">
                                <span class="text-slate-300">Overhead</span>
                                <span class="text-slate-500 text-sm">(balance: ৳{{ number_format($overheadBalance, 0) }})</span>
                            </label>
                            <label class="flex items-center gap-3 p-3 rounded-xl border border-slate-600 hover:bg-slate-800/50 cursor-pointer">
                                <input type="radio" name="funded_from" value="profit" {{ old('funded_from') === 'profit' ? 'checked' : '' }} class="rounded border-slate-600 bg-slate-800 text-sky-500 focus:ring-sky-500">
                                <span class="text-slate-300">Profit pool</span>
                                <span class="text-slate-500 text-sm">(balance: ৳{{ number_format($profitBalance ?? 0, 0) }})</span>
                            </label>
                            <label class="flex items-center gap-3 p-3 rounded-xl border border-slate-600 hover:bg-slate-800/50 cursor-pointer">
                                <input type="radio" name="funded_from" value="investment" {{ old('funded_from') === 'investment' ? 'checked' : '' }} class="rounded border-slate-600 bg-slate-800 text-sky-500 focus:ring-sky-500">
                    ...
                    <div class="flex gap-3 pt-2">
                        ...
                        <button type="submit" class="px-4 py-2.5 rounded-xl bg-sky-500 hover:bg-sky-600 text-white font-medium">Record expense</button>
                    </div>
                </form>
            </div>
        </div>
