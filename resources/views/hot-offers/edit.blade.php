<x-app-layout>
    ...
    <div class="max-w-2xl">
        <div class="mb-6">
            ...
            <h1 class="text-2xl font-semibold theme-text-primary mt-1">Edit Hot Offer</h1>
        </div>
        <div class="theme-card-bg-only theme-border border rounded-2xl p-6">
            <form action="{{ route('hot-offers.update', $hotOffer) }}" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title', $hotOffer->title) }}" required class="w-full rounded-xl theme-input-bg border theme-border theme-text-primary px-4 py-2.5 theme-input-focus">
                ...
                <div>
                    ...
                    <input type="number" name="price" id="price" value="{{ old('price', $hotOffer->price) }}" step="0.01" min="0" placeholder="Leave empty for no price" class="w-full rounded-xl theme-input-bg border theme-border theme-text-primary px-4 py-2.5 theme-input-focus">
                    @error('price')<p class="text-red-400 text-xs mt-1">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label for="cta_text" class="block text-sm font-medium theme-text-secondary mb-1">CTA Button Text *</label>
                    <input type="text" name="cta_text" id="cta_text" value="{{ old('cta_text', $hotOffer->cta_text) }}" required class="w-full rounded-xl theme-input-bg border theme-border theme-text-primary px-4 py-2.5 theme-input-focus">
                    @error('cta_text')<p class="text-red-400 text-xs mt-1">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="hidden" name="is_active" value="0">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $hotOffer->is_active) ? 'checked' : '' }} class="rounded theme-border theme-input-bg text-orange-500 focus:ring-orange-500">
